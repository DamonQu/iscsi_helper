...
func Discovery(tp, iface string) ([]string, error) {
	baseArgs := []string{"-m", "discovery", "-t", "sendtargets", "-p", tp, "-I", iface, "--discover"}
	out, err := iscsiCmd(baseArgs...)
	if err != nil {
		return nil, fmt.Errorf("failed to sendtargets to portal %s, error: %v", tp, err)
	}
	lines := strings.Split(out, "\n")
	if len(lines) == 0 {
		return nil, fmt.Errorf("no targets found in portal %s", tp)
	}

	var targetIqnList []string
	for _, line := range lines {
		if len(line) != 0 {
			...
			targetIqnList = append(targetIqnList, parts[1])
		}
	}
	return targetIqnList, err
}
